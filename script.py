        ...
        if account:
            ...
            try:
                ...
                try:
                    ...
                    # Handle reserved keyword quoting
                    def quote_identifier(col):
                        if col.lower() in ('type',):  # Add more reserved words if needed
                            return f'"{col.upper()}"'
                        return f'"{col.upper()}"'

                    columns_sql_parts = []
                    for col in fieldnames:
                        col_name = quote_identifier(col)
                    ...
                    # Same quoting for insert
                    column_list = ', '.join([quote_identifier(c) for c in fieldnames])
